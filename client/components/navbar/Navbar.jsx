        ...
        <div className="w-full m-0 p-0 flex justify-center items-center">
            <div className="w-full max-w-[1100px] m-0 p-3 flex justify-between items-center px-6">
                ...
                <div className="xs:hidden md:flex justify-between items-center gap-7">
                    ...
                    {!user ?
                        <Link href="/login" className="cursor-pointer text-[#1576DB] hover:bg-[#1576DB] hover:text-white border px-5 py-1 rounded-full  border-[#1576DB] font-bold">Sign In</Link>
                        : 
                        <>
                            <Link href="/blog/create" className="cursor-pointer text-[#183B56] hover:text-[#1576DB]">New</Link>
                            <Link href={`/profile/${user?.id}`} className="cursor-pointer text-[#1576DB] hover:text-rose-500 border-[#1576DB] font-bold flex flex-row items-center justify-center capitalize"><RxAvatar />&nbsp;{firstName}</Link>
                            <button onClick={()=>{}} className="cursor-pointer text-[brown] hover:bg-[#813333] hover:text-white border px-5 py-1 rounded-full  border-[brown] font-bold">LogOut</button>
                        </>
                    }
                </div>
